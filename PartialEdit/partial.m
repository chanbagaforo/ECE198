...
%
% Created by Victor Gabriel C. Garcia for CoE 198 2s1920
...
if ~strcmp(file,"0") & ~strcmp(path,"0")
    ...
    for i = 1:numel(file)
        ...
        for j = 1:numel(sbin) % per partial
            [amp,freq,Fs,DispLen,Sample_Offset] = modalest(filename,sbin(j),ebin(j));
            ...
            f = figure('Name',tmp); % new figure for given partial
            tg = uitabgroup(f); % tab group ui element
